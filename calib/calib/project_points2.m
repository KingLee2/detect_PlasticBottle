...
if nargout > 1,
    dxdom = zeros(2*n,3);
    dxdom(1:2:end,:) = ((inv_Z')*ones(1,3)) .* dYdom(1:3:end,:) + bb .* dYdom(3:3:end,:);%公式推导对的。
...
if nargout > 1,
    dr2dom = 2*((x(1,:)')*ones(1,3)) .* dxdom(1:2:end,:) + 2*((x(2,:)')*ones(1,3)) .* dxdom(2:2:end,:);%公式推导对的。
...
if nargout > 1,
    dcdistdom = k(1) * dr2dom + k(2) * dr4dom + k(5) * dr6dom;%公式推导对的。
...
if length(f)>1,
    ...
    if nargout > 1,
        ...
        dxpdk = (coeff*ones(1,5)) .* dxd3dk; % 对K求导
        dxpdalpha = (coeff) .* dxd3dalpha; % 对ALPHA求导
        dxpdf = zeros(2*n,2);
        dxpdf(1:2:end,1) = xd3(1,:)';% 对f求导 xp = xd3.*f+c 所以变成xd3'
