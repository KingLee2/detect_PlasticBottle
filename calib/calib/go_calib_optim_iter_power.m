...
comp_ext_calib_power; % !!计算第一次外参,  by eros
...
while ((change > 1e-15) && iter < MaxIter),%
    ...
    for kk = ind_active, %1:n_ima,
        ...
        if ~est_aspect_ratio,
            ...
            % square pixels) 倾斜参数?
            [x,dxdom,dxdT,dxdf,dxdc,dxdk,dxdalpha] = project_points2_power(X_kk,omckk,Tckk,f(1),c,k,alpha);%把3D空间投影到2D X 空间坐标, 
            dxdf = repmat(dxdf,[1 2]);
        else
            [x,dxdom,dxdT,dxdf,dxdc,dxdk,dxdalpha] = project_points2_power(X_kk,omckk,Tckk,f,c,k,alpha);
        end;
    ...
    param_innov = alpha_smooth2*JJ2_inv*ex3;% 计算更新参数
    ...
    if recompute_extrinsic,
        ...
        for kk =ind_active, %1:n_ima,
            ...
            eval(['X_kk = X_' num2str(kk) ';']);
            eval(['x_kk = x_' num2str(kk) ';']);
            [omc_current,Tc_current] = compute_extrinsic_init_power(x_kk,X_kk,fc_current,cc_current,kc_current,alpha_current);%外参计算初始计算
            [omckk,Tckk,Rckk,JJ_kk] = compute_extrinsic_refine_power(omc_current,Tc_current,x_kk,X_kk,fc_current,cc_current,kc_current,alpha_current,MaxIter2,thresh_cond);% 外参修正
            if check_cond,
                if (cond(JJ_kk)> thresh_cond),
                    active_images(kk) = 0;
                    fprintf(1,'\nWarning: View #%d ill-conditioned. This image is now set inactive. (note: to disactivate this option, set check_cond=0)\n',kk);
...
for kk = ind_active,
    ...
    if ~est_aspect_ratio,
        [x,dxdom,dxdT,dxdf,dxdc,dxdk,dxdalpha] = project_points2_power(X_kk,omckk,Tckk,fc(1),cc,kc,alpha_c);
        dxdf = repmat(dxdf,[1 2]);
    else
        [x,dxdom,dxdT,dxdf,dxdc,dxdk,dxdalpha] = project_points2_power(X_kk,omckk,Tckk,fc,cc,kc,alpha_c);
    end;
