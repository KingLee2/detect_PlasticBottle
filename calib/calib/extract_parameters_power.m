...
if ~exist('solution_error')
   solution_error = zeros(6*n_ima + param_counter,1);
end;
...
kc_error = solution_error(6:param_counter);
...
for kk = 1:n_ima,
   ...
   if active_images(kk),   
      ...
      omckk_error = solution_error(param_counter_long+6*(kk-1) + 1:param_counter_long+6*(kk-1) + 3); 
      Tckk_error = solution_error(param_counter_long+6*(kk-1) + 4:param_counter_long+6*(kk-1) + 6);
