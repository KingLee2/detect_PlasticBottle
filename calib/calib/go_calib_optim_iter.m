...
comp_ext_calib; % !!计算第一次外参,  by eros
...
while ( iter < MaxIter),%(change > 1e-15) &&
    ...
    for kk = ind_active, %1:n_ima,
        ...
        omckk = param(15+6*(kk-1) + 1:15+6*(kk-1) + 3); % 获取外参 旋转向量
        
        Tckk = param(15+6*(kk-1) + 4:15+6*(kk-1) + 6); % 外参 平移向量 X,Y,Z 3个方向
        ...
        if ~est_aspect_ratio,
            ...
            % square pixels) 倾斜参数?
            [x,dxdom,dxdT,dxdf,dxdc,dxdk,dxdalpha] = project_points2(X_kk,omckk,Tckk,f(1),c,k,alpha);%把3D空间投影到2D X 空间坐标, 
    ...
    if recompute_extrinsic,
        ...
        for kk =ind_active, %1:n_ima,
            ...
            eval(['X_kk = X_' num2str(kk) ';']);
            eval(['x_kk = x_' num2str(kk) ';']);
            [omc_current,Tc_current] = compute_extrinsic_init(x_kk,X_kk,fc_current,cc_current,kc_current,alpha_current);%外参计算初始计算
            [omckk,Tckk,Rckk,JJ_kk] = compute_extrinsic_refine(omc_current,Tc_current,x_kk,X_kk,fc_current,cc_current,kc_current,alpha_current,MaxIter2,thresh_cond);% 外参修正
            if check_cond,
                if (cond(JJ_kk)> thresh_cond),
                    active_images(kk) = 0;
                    fprintf(1,'\nWarning: View #%d ill-conditioned. This image is now set inactive. (note: to disactivate this option, set check_cond=0)\n',kk);
