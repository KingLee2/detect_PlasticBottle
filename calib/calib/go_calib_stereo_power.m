...
% by eros 从原来的5e -6 增加到5e-8
while (iter <= MaxIter),%(change > 5e-8)&
    ...
    for kk = 1:n_ima,
        ...
        if active_images(kk),
            ...
            Jkk = sparse(4*Nckk,new_param_cont+(1+n_ima)*6);% 原始算法， 参数20个 fc cc kc alpha_c 2+2+5+1
            ekk = zeros(4*Nckk,1);
            
            
            if ~est_aspect_ratio_left,
                [xl,dxldomckk,dxldTckk,dxldfl,dxldcl,dxldkl,dxldalphal] = project_points2_power(Xckk,omckk,Tckk,fc_left(1),cc_left,kc_left,alpha_c_left);
                dxldfl = repmat(dxldfl,[1 2]);
            else
                [xl,dxldomckk,dxldTckk,dxldfl,dxldcl,dxldkl,dxldalphal] = project_points2_power(Xckk,omckk,Tckk,fc_left,cc_left,kc_left,alpha_c_left);
            end;
        
            
            ekk(1:2*Nckk) = xlkk(:) - xl(:);
            
            Jkk(1:2*Nckk,6*(kk-1)+7+new_param_cont:6*(kk-1)+7+2+new_param_cont) = sparse(dxldomckk);
            Jkk(1:2*Nckk,6*(kk-1)+7+3+new_param_cont:6*(kk-1)+7+5+new_param_cont) = sparse(dxldTckk);
            
            Jkk(1:2*Nckk,1:2) = sparse(dxldfl);
            Jkk(1:2*Nckk,3:4) = sparse(dxldcl);
            Jkk(1:2*Nckk,5) = sparse(dxldalphal);
            Jkk(1:2*Nckk,6:param_counter) = sparse(dxldkl);
            
            
            % Project the structure onto the right view:
            
            % compose_motion ==> omckk Tckk 左相机的R T ||  om T  左右相机的R T
            ...
            if ~est_aspect_ratio_right,
                [xr,dxrdomr,dxrdTr,dxrdfr,dxrdcr,dxrdkr,dxrdalphar] = project_points2_power(Xckk,omr,Tr,fc_right(1),cc_right,kc_right,alpha_c_right);
                dxrdfr = repmat(dxrdfr,[1 2]);
            else
                [xr,dxrdomr,dxrdTr,dxrdfr,dxrdcr,dxrdkr,dxrdalphar] = project_points2_power(Xckk,omr,Tr,fc_right,cc_right,kc_right,alpha_c_right);
            end;
            ...
            Jkk(2*Nckk+1:end,1+new_param_cont:3+new_param_cont) =  sparse(dxrdom);
            Jkk(2*Nckk+1:end,4+new_param_cont:6+new_param_cont) =  sparse(dxrdT);
            
            
            Jkk(2*Nckk+1:end,6*(kk-1)+7+new_param_cont:6*(kk-1)+7+2+new_param_cont) = sparse(dxrdomckk);
            Jkk(2*Nckk+1:end,6*(kk-1)+7+3+new_param_cont:6*(kk-1)+7+5+new_param_cont) = sparse(dxrdTckk);
            
            Jkk(2*Nckk+1:end,26:27) = sparse(dxrdfr);
            Jkk(2*Nckk+1:end,28:29) = sparse(dxrdcr);
            Jkk(2*Nckk+1:end,30) = sparse(dxrdalphar);
            Jkk(2*Nckk+1:end,31:new_param_cont) = sparse(dxrdkr);
    ...
    kc_left = param(6:25);%changed
    fc_right = param(26:27);
    cc_right = param(28:29);
    alpha_c_right = param(30);
    kc_right = param(31:50);
    ...
    om = param(1+new_param_cont:3+new_param_cont);
    T = param(4+new_param_cont:6+new_param_cont);
    ...
    for kk = 1:n_ima;
        ...
        if active_images(kk),
            ...
            omckk = param(6*(kk-1)+7+new_param_cont:6*(kk-1)+7+2+new_param_cont);
            Tckk = param(6*(kk-1)+7+3+new_param_cont:6*(kk-1)+7+5+new_param_cont);
...
sigma_x = std(e(:));
param_error = zeros(new_param_cont + (1+n_ima)*6,1);
...
for kk = 1:n_ima;
    ...
    if active_images(kk),
        ...
        omckk_error = param_error(6*(kk-1)+7+new_param_cont:6*(kk-1)+7+2+new_param_cont);
        Tckk = param_error(6*(kk-1)+7+3+new_param_cont:6*(kk-1)+7+5+new_param_cont);
...
kc_left_error = param_error(6:25);
fc_right_error = param_error(26:27);
cc_right_error = param_error(28:29);
alpha_c_right_error = param_error(30);
kc_right_error = param_error(31:new_param_cont);
...
om_error = param_error(1+new_param_cont:3+new_param_cont);
T_error = param_error(4+new_param_cont:6+new_param_cont);
...
fprintf(1,'Skew:             alpha_c_left = [ %3.5f ] ?[ %3.5f  ]   => angle of pixel axes =021ujy6tr5de43swaq %3.5f ?%3.5f degrees\n',[alpha_c_left;alpha_c_left_error],90 - atan(alpha_c_left)*180/pi,atan(alpha_c_left_error)*180/pi);
