...
figure(2); hold on;
for count = 1:4,
    [xi,yi] = ginput4(1);%输入一个点
    [xxi] = cornerfinder([xi;yi],I,winty,wintx);% 找到精确的交叉点
    xi = xxi(1);
    yi = xxi(2);
    figure(2);
...
[Xc,good,bad,type] = cornerfinder([x';y'],I,winty,wintx); % the four corners 重找4个点。再精确一次

x = Xc(1,:)';
y = Xc(2,:)';


% Sort the corners: 通过中心点， 来确定各点顺序
x_mean = mean(x);
y_mean = mean(y);
x_v = x - x_mean;
y_v = y - y_mean;
...
ind = ind([4 3 2 1]); %-> New: the Z axis is pointing uppward 颠倒顺序
% 变前
% 4----1
% |    |
% 3----2
...
XX = Homo*pts;%重算所有的点
XX = XX(1:2,:) ./ (ones(2,1)*XX(3,:)); %让Z轴为1 转回平面
...
if quest_distort,
    % Estimation of focal length: 估计焦距
...
% 
% grid_pts = cornerfinder(XX,I,winty,wintx); %%% Finds the exact corners at every points! 再重找所有的点，基础为旋转矩阵算出来的点
% 
% 
% 
